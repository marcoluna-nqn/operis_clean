...
// Escritura atÃ³mica en Documentos\Bitacora\exports (Win) o AppDocs/Bitacora/exports (Android).
...
class ExportXlsxService {
  ...
  Future<ExportResult> exportToXlsx({
    ...
    void ensure() {
      if (cancelToken?.canceled == true) {
        throw const FileSystemException('ExportaciÃ³n cancelada');
      }
    }
    ...
    try {
      ...
      if (rows.isEmpty) {
        // Al menos una fila para evitar XLSX â€œvacÃ­oâ€.
        final excelRow = 2;
      ...
      // Guardado atÃ³mico + copia en Temp
      step(0.86, 'Guardando');
  ...
  /// â€œGuardar comoâ€¦â€ compatible con file_selector antiguos: usa getDirectoryPath.
  ...
  Future<void> shareQuick(XFile xf, {File? fallbackReveal}) async {
    try {
      ...
      if (!await f.exists() || await f.length() == 0) return; // evita â€œsolo textoâ€
      await Share.shareXFiles([xf], text: 'Archivo XLSX generado con BitÃ¡cora');
    } catch (_) {
  ...
  // ===== Hoja â€œFotosâ€ agrupada por fila =====
