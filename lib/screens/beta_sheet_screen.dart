...
// Planilla con persistencia inmediata y diseño industrial 2025.
// Fixes: PopScope en lugar de WillPopScope, textScalerOf, Color.withValues,
// _insertHeadersAt => Future<void>, guardados “fire-and-forget”.
...
class _BetaSheetScreenState extends State<BetaSheetScreen>
  ...
  // Datos
  late final List<String> _headers = List<String>.generate(widget.columns, (_) => '');
  ...
  String get _sheetKey =>
      (widget.sheetId != null && widget.sheetId!.isNotEmpty) ? widget.sheetId! : kDefaultSheetId;
      ...
      if (stored != null && stored.sheetId == id) {
        ...
        for (var r = 0; r < stored.rows.length; r++) {
          ...
          if (_rows[r].cells.length < _headers.length) {
            _rows[r].cells.addAll(List.filled(_headers.length - _rows[r].cells.length, ''));
          }
  ...
  // ----- Encabezados -----
  Future<void> _insertHeadersAt(List<String> names, {int? at}) async {
    ...
    await _ensureDirtySaveNow(); // persistencia inmediata
  }
  ...
  Future<void> _addHeadersInteractive() async {
    ...
    await _insertHeadersAt(names);
    Feel.tap();
    _toast(names.length == 1 ? 'Encabezado agregado' : '${names.length} encabezados agregados');
  }
            ...
            return Padding(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                Row(children: [
                  ...
                  const Spacer(),
                  TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Restablecer')),
                ]),
                ...
                Align(
                  ...
                  child: Text(
                    ...
                    style: Theme.of(ctx).textTheme.bodySmall?.copyWith(color: cs.outline),
                  ),
                ),
              ]),
            );
  ...
  // ===== Captura normal sin temporizador =====
  Future<String?> _capturePhotoNormal() async {
    try {
      ...
      final photosDir = io.Directory(p.join(docs.path, 'gridnote_photos', _sheetKey));
      if (!photosDir.existsSync()) {
        photosDir.createSync(recursive: true);
      }
      final ext = p.extension(shot.path);
      final fname = 'ph_${DateTime.now().millisecondsSinceEpoch}${ext.isEmpty ? '.jpg' : ext}';
      final dest = p.join(photosDir.path, fname);
      await io.File(shot.path).copy(dest);
      return dest;
    } catch (e) {
  ...
  // >>> Agregar foto rápida (sin countdown) <<<
  Future<void> _addPhotoQuick(int rowIndex) async {
    ...
    try {
      final path = await _capturePhotoNormal();
      if (!mounted || path == null || path.isEmpty) return;

      if (_totalPhotos() >= kMaxPhotosTotal || _rows[rowIndex].photos.length >= kMaxPhotosPerRow) {
  ...
  Future<void> _markLocationAdaptive(int rowIndex) async {
    await _busy('Obteniendo GPS…', () async {
      try {
        ...
        final acc = fix.accuracyMeters != null ? ' ±${fix.accuracyMeters!.toStringAsFixed(0)}m' : '';
  ...
  Future<void> _newSheetFlow() async {
    ...
    final name = nameCtrl.text.trim().isEmpty ? 'Nueva planilla' : nameCtrl.text.trim();
  ...
  PreferredSizeWidget _buildAppBarMaterial(BuildContext context) {
    ...
    return AppBar(
      ...
      title: Row(children: [
        const ExcludeSemantics(child: Icon(Icons.table_view_rounded, size: 20)),
        const SizedBox(width: 8),
        Flexible(
          child: Text(
            _title,
  ...
  double _blurSigma() => Theme.of(context).platform == TargetPlatform.android ? 8 : 18;

  // Barra inferior RESPONSIVA
  Widget _bottomActions() {
    return LayoutBuilder(builder: (ctx, cts) {
      final scale = MediaQuery.textScalerOf(ctx).scale(1.0);
      final narrow = cts.maxWidth < 420 || scale > 1.10;

      Text oneLine(String s) =>
          Text(s, maxLines: 1, softWrap: false, overflow: TextOverflow.ellipsis);
      ...
      return Row(children: [
        ...
        Flexible(
          child: OutlinedButton.icon(
            onPressed: _addHeadersInteractive,
            icon: const Icon(Icons.view_column),
            label: oneLine('Añadir encabezado'),
          ),
        ),
        const SizedBox(width: 10),
        Expanded(
  ...
  Widget _buildGridArea(Color divider, Color headerBg, Color rowBg) {
    ...
    return LayoutBuilder(builder: (context, constraints) {
      ...
      final Color headerBandBg = headerBg;
      ...
      final list = ListView.builder(
        ...
        itemBuilder: (_, i) {
          ...
          final bool shouldFlash = _flashRows.contains(realIndex);
          final Color targetColor =
          shouldFlash ? cs.primaryContainer.withValues(alpha: 0.20) : baseColor;

          final rowCore = Column(children: [
            Row(children: [
              for (var c = 0; c < _headers.length; c++)
                _Cell(
                  width: cellWidth,
                  ...
                  return InkWell(
                    ...
                    child: AnimatedSwitcher(
                      ...
                      child: (r.lat == null)
                        ...
                        width: cts.maxWidth,
                        child: _CoordPill(
                          key: ValueKey('lat-${r.lat}-${acc?.toStringAsFixed(0)}'),
                      ...
                      child: GestureDetector(
                        ...
                        child: AppleLocationPulse(
                          ...
                          onFix: (f) async {
                            ...
                            await _ensureDirtySaveNow();
                            final a =
                            f.accuracyMeters != null ? ' ±${f.accuracyMeters!.toStringAsFixed(0)}m' : '';
              ...
              _Cell(
                ...
                child: Center(
                  child: InkWell(
                    onTap: () => _confirmDeleteRow(realIndex),
                    borderRadius: BorderRadius.circular(8),
                    child: const Padding(
          ...
          final rowAnimated = RepaintBoundary(
            child: TweenAnimationBuilder<Color?>(
              duration: kMotion,
              curve: kAppleCurve,
              tween: ColorTween(begin: baseColor, end: targetColor),
              builder: (_, color, child) => ColoredBox(color: color ?? baseColor, child: child!),
              child: rowCore,
            ),
          );
    ...
    final seg = CupertinoSegmentedControl<int>(
      children: const <int, Widget>{
        0: Padding(
          ...
          child: Text('Todos', maxLines: 1, softWrap: false, overflow: TextOverflow.ellipsis),
        ),
        1: Padding(
          padding: EdgeInsets.symmetric(horizontal: 8, vertical: 6),
          child: Text('Fotos', maxLines: 1, softWrap: false, overflow: TextOverflow.ellipsis),
        ),
        2: Padding(
          padding: EdgeInsets.symmetric(horizontal: 8, vertical: 6),
          child: Text('GPS', maxLines: 1, softWrap: false, overflow: TextOverflow.ellipsis),
        ),
      },
        ...
        child: Center(
          child: Container(
            ...
            child: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                SizedBox(width: 24, height: 24, child: Center(child: spinner)),
                const SizedBox(width: 12),
                Text(_busyLabel ?? 'Procesando…', style: Theme.of(context).textTheme.bodyMedium),
              ],
            ),
          ),
        ),
  ...
  Widget _withShortcuts(Widget child) {
    return Shortcuts(
      shortcuts: <LogicalKeySet, Intent>{
        LogicalKeySet(LogicalKeyboardKey.meta, LogicalKeyboardKey.keyN): const _AddRowIntent(),
        LogicalKeySet(LogicalKeyboardKey.meta, LogicalKeyboardKey.keyF): const _OpenFilterIntent(),
        LogicalKeySet(LogicalKeyboardKey.meta, LogicalKeyboardKey.keyE): const _ExportIntent(),
        LogicalKeySet(LogicalKeyboardKey.meta, LogicalKeyboardKey.delete): const _DeleteLastRowIntent(),
      },
      child: Actions(
        actions: <Type, Action<Intent>>{
          ...
          _DeleteLastRowIntent: CallbackAction<_DeleteLastRowIntent>(onInvoke: (_) {
            if (_rows.isNotEmpty) {
    ...
    final content = Stack(
      children: [
        ...
        Positioned(
          ...
          right: 0,
          bottom: 0,
          child: _frostedBottomBar(_bottomActions()),
        ),
      ],
    );
  ...
  Widget build(BuildContext context) {
    ...
    final Color headerBg = Colors.black;
    ...
    final page = isIOS
      ...
      bottomNavigationBar: _frostedBottomBar(_bottomActions()),
      body: _withShortcuts(
        Column(
  ...
  Future<void> _rehydrateFromCrashGuard() async {
    ...
    if (changed) await _ensureDirtySaveNow();
  }
      ...
      builder: (ctx) {
        ...
        return SafeArea(
          child: Padding(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              Row(children: [
                Text('Fotos (${photos.length})', style: Theme.of(ctx).textTheme.titleMedium),
                const Spacer(),
                IconButton(
                  onPressed: () async {
                    Navigator.pop(ctx);
                ...
                if (!SpeechService.I.isAvailable) {
                  if (mounted) {
                    ScaffoldMessenger.of(context)
                        .showSnackBar(const SnackBar(content: Text('Dictado no disponible.')));
                  }
                  return;
                }
              }
            ...
            return Padding(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                CupertinoTextField(
                  ...
                  autofocus: true,
                  placeholder:
                  placeholder.isEmpty ? (listening && livePartial.isNotEmpty ? '… $livePartial' : '') : placeholder,
                  padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 12),
                  onChanged: (_) {
                ...
                Column(
                  ...
                  children: [
                    ...
                    Text('Guardada en la fila',
                        style: Theme.of(ctx).textTheme.labelSmall?.copyWith(color: cs.outline)),
                  ],
                ),
                const SizedBox(width: 8),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: fill,
        border: Border(
          left: isBandStart ? BorderSide(color: divider, width: 2.0) : BorderSide.none,
  ...
  Widget build(BuildContext context) {
    ...
    Widget popBadge(int value) => AnimatedSwitcher(
      ...
      child: Container(
        ...
        child:
        Text('$value', style: const TextStyle(fontSize: 11, color: Colors.white, fontWeight: FontWeight.w700)),
      ),
    );
...
class _GlassHudState extends State<_GlassHud> with SingleTickerProviderStateMixin {
  ...
  late final Animation<double> _fade = CurvedAnimation(parent: _c, curve: kAppleCurve);
  late final Animation<Offset> _slide =
  Tween(begin: const Offset(0, 0.1), end: Offset.zero).chain(CurveTween(curve: kAppleCurve)).animate(_c);
