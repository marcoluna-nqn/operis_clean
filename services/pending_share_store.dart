...
/// Persiste en SharedPreferences (puede migrarse a JSON atómico si crece).
class PendingShareStore {
  ...
  Future<void> enqueueFiles(
    ...
    final boundedPriority = priority < 0
        ? 0
        : (priority > 100 ? 100 : priority); // evita `num` de clamp()

    final item = PendingShare(
      files: files
          .map((f) =>
          PendingShareFile(path: f.path, name: f.name, mimeType: f.mimeType))
          .toList(),
      subject: subject,
      text: text ?? '',
      createdAt: now,
    ...
    if (!already) {
      if (collapseWithSame) {
        final int idx = all
            .lastIndexWhere((it) => it.subject == item.subject && it.text == item.text);
        if (idx != -1) {
  ...
  Future<void> processQueueIfOnline() async {
    ...
    try {
      ...
      // Purga vencidos.
      all.removeWhere((it) => it.expiresAt != null && now.isAfter(it.expiresAt!));
      ...
      for (final PendingShareFile f in head.files) {
        ...
        if (mt == null ||
          ...
          // Si no hay mime válido, dejamos pasar sólo XLSX/CSV/PDF/imagenes por extensión.
      ...
      if (validFiles.isEmpty) {
        ...
        all.removeAt(0);
        await _save(all);
        // Intentar inmediatamente con el siguiente.
        await _setNextAttemptAt(now);
        // Si hay más, reintenta en esta misma pasada.
        if (all.isNotEmpty) {
          _processing = false;
          await processQueueIfOnline();
          return;
      ...
      if (res.status == ShareResultStatus.success) {
        ...
        // Remove y reset backoff.
        all.removeAt(0);
        await _save(all);
        await _setNextAttemptAt(now);
        // Si hay más, seguimos en la misma corrida.
        if (all.isNotEmpty) {
          _processing = false;
          await processQueueIfOnline();
        }
        ...
        final int nextSec =
        _computeBackoffWithJitter(head.attempts + 1, minBackoffSec, maxBackoffSec);
      ...
      // Cualquier error: incrementa intentos de head (si existía) y reprograma.
      try {
        final List<PendingShare> all = await _load();
        if (all.isNotEmpty) {
          ...
          final int nextSec =
          _computeBackoffWithJitter(head.attempts, minBackoffSec, maxBackoffSec);
  ...
  Future<List<PendingShare>> _load() async {
    ...
    // Migración blanda desde v1 si existiera (no falla si no hay).
  ...
  Future<void> _save(List<PendingShare> items) async {
    final SharedPreferences p = await SharedPreferences.getInstance();
    final List<String> lines = items.map((e) => jsonEncode(e.toJson())).toList();
  ...
  int _computeBackoffWithJitter(int attempts, int minSec, int maxSec) {
    ...
    final int base = (minSec * factor).clamp(minSec, maxSec);
    ...
    final int offset = DateTime.now().microsecondsSinceEpoch % range;
    final int res = low.floor() + offset;
    return res.clamp(minSec, maxSec);
  }
}
